...
- [Master facility lists](https://ahadi-analytics.github.io/snt-code-library/english/library/data/health_facilities/master_facility_lists.html) - matching DHIS2 to the MFL.
:::
...
list. `fuzzy_match_facilities()` runs a staged matching pipeline:

1. exact match,
2. match after `standardize_names()` normalisation,
...
`calculate_match_stats()` summarises the same results by method so we
can compare matching strategies side by side. The matched facility
table is the bridge between a DHIS2 export and the spatial master
facility list used in
...
`dhis2_map()` renames a DHIS2 export's columns using a name-mapping
dictionary, so we can keep the upstream column labels intact on disk
and only remap to SNT names when we load the data. It's a small but
critical step in every DHIS2 import workflow, and a natural neighbour
of `check_snt_var()` above - one renames, the other validates that the
renamed columns hit the canonical schema.

> **Used in:** the
> [DHIS2 import and preprocessing](https://ahadi-analytics.github.io/snt-code-library/english/library/data/routine_cases/import.html)
> chapter of the SNT Code Library walks through where this fits in the
> end-to-end DHIS2 cleaning pipeline. `dhis2_map()` is the helper that
> performs the rename step described there.
...
Pair it with `check_snt_var()` immediately after the rename to confirm
the resulting column names match the canonical SNT schema before
joining DHIS2 to anything else.
