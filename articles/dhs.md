...
household-level intervention coverage, child mortality and biomarker
data across most malaria-endemic countries.
...
is a thin wrapper around the DHS API’s metadata endpoint, returning the
indicator catalogue as a tibble.
...
[`get_dhs_data()`](https://ahadi-analytics.github.io/sntutils/reference/get_dhs_data.md)
registers a directory of those parquet files as DuckDB views, returning
a list ready for `dplyr` / `dbplyr` queries.
...
there for the 20% where the country team needs something the DHS chapter
never published.
