...
The `download_chirps()` function allows you to fetch CHIRPS monthly rainfall raster data for any supported region and time period. It pulls data directly from the [UCSB Climate Hazards Group](https://www.chc.ucsb.edu/data/chirps) FTP archive and supports automatic unzipping. Only `.tif.gz` monthly rasters are supported, and the function avoids re-downloading existing files. To view all supported CHIRPS datasets, use `chirps_options()`. To check the available years and months for a specific CHIRPS dataset (e.g., africa_monthly), use the `check_chirps_available()` function.

```{r}
#| label: chirps-download-example
#| eval: false
#| include: true
...
```{r}
#| label: process-raster-collection
#| eval: false
#| include: true
...
The `sntutils::calculate_reporting_metrics()` function computes the completeness of routine health data reporting by assessing whether health facilities have submitted valid data for a specified set of indicators (`key_indicators`) over time. It then calculates reporting rates for one or more target indicators (`vars_of_interest`) based on this information.
...
- `key_indicators` = a set of variables used to determine whether a facility is active
e.g. "test", "treat", "conf", "pres", "allout"
...
- $o_{a,t}$ (observed) = number of facilities in $a$ that reported *any* value in `vars_of_interest` during $t$
- $e_{a,t}$ (expected) = number of facilities in $a$ whose *first-eve*r report on any `key_indicators` occurred on or before $t$ (i.e. expected to report)
...
```{r}
#| label: reporting-metrics-scenario1
#| eval: false
#| include: true
# Example data with inconsistent admin names
...
```{r}
#| label: reporting-metrics-scenario1-output
#| echo: false
library(sntutils)
...
sntutils::calculate_reporting_metrics(
  ...
  key_indicators = c(
    "allout",
    "test", # Used to define eligibility
    "treat",
    "conf",
    "pres"
  )
...
```{r}
#| label: reporting-metrics-scenario2
#| eval: false
#| include: true
...
```{r}
#| label: reporting-metrics-scenario2-output
#| echo: false
#| message: false
#| warning: false
...
```{r}
#| label: reporting-metrics-scenario3
#| eval: false
#| include: true
...
```{r}
#| label: reporting-metrics-scenario3-output
#| echo: false
# Evaluate reporting completeness over time
sntutils::calculate_reporting_metrics(
  data = sl_dhis2,
...
```{r}
#| label: reporting-rate-plot-scenario1
#| eval: false
#| include: true
reporting_rate_plot(
  data = sl_dhis2,
...
```{r}
#| label: reporting-rate-plot-scenario1-output
#| echo: false
#| message: true
#| warning: false
#| fig-height: 8
...
```{r}
#| label: reporting-rate-plot-scenario2
#| eval: false
#| include: true
reporting_rate_plot(
  data = sl_dhis2,
...
```{r}
#| label: reporting-rate-plot-scenario2-output
#| echo: false
#| message: true
#| warning: false
#| fig-height: 8
...
```{r}
#| label: reporting-rate-plot-scenario3
#| eval: false
#| include: true
...
```{r}
#| label: reporting-rate-plot-scenario3-output
#| echo: false
#| message: true
#| warning: false
#| fig-height: 8
...
inconsistencies in malaria surveillance data by validating that upstream
events (inputs) are greater than or equal to downstream events (outputs)
in the care cascade. This is useful for data quality assessment.
...
```{r}
#| label: consistency-check-examples
#| eval: false
#| include: true
# Check consistency between tests (inputs) and confirmed cases (outputs)
consistency_check(
  sl_dhis2,
  inputs = c("test"),
  outputs = c("conf")
...
```{r}
#| label: consistency-check-output
#| echo: false
#| message: true
#| warning: false
#| fig-height: 6
...
# randomly select 60 rows to modify
set.seed(2025)
rows_to_modify <- sl_dhis2 |>
  dplyr::sample_n(17) |>
  dplyr::mutate(
    test = sample(500:20000, 17, replace = TRUE, ),
    conf = test + sample(100:20000, 17, replace = TRUE)
  )

# overwrite those rows in sl_dhis2
sl_dhis2_v2 <- sl_dhis2 |>
  dplyr::rows_update(rows_to_modify, by = c("hf_uid", "year_mon"))
...
The `detect_outliers()` function returns a table with outlier results for each row in your dataset. The key columns of interest are record_id, the value being checked, and the method-specific flags: outliers_iqr, outliers_median, and outliers_mean. Each method marks the value as either "outlier" or "normal value". You can join this output back to your original data using record_id to flag values for review or action.

```{r}
#| label: detect-outliers-output
outlier_results |>
    dplyr::select(
        record_id, value,
        outliers_iqr,
...
```{r}
#| label: outlier-plot-save
#| include: false
ggplot2::ggsave(
  here::here(
    "man/figures-readme/outlier_plot.png"
  ),
  plot = plots$iqr,
  width = 16,
  height = 12
...
ggplot2::ggsave(
  here::here(
    "man/figures-readme/outlier_plot2.png"
  ),
  plot = plots$median,
  width = 16,
  height = 12
...
ggplot2::ggsave(
  here::here(
    "man/figures-readme/outlier_plot3.png"
  ),
  plot = plots$mean,
  width = 16,
  height = 12
...
```{r}
#| label: outlier-plot-iqr-display
#| eval: false
plots$iqr
```
...
```{r}
#| label: outlier-plot-median-display
#| eval: false
plots$median
```
...
```{r}
#| label: outlier-plot-mean-display
#| eval: false
plots$mean
```
...
```{r}
#| label: translate-text-vec-example
#| eval: false
#| include: true
library(dplyr)
...
```{r}
#| label: translate-text-vec-output
#| echo: false
#| message: false
#| warning: false
library(dplyr)
...
```{r}
#| label: translated-yearmon-examples
#| eval: false
#| include: true
...
```{r}
#| label: numeric-formatting-examples
#| eval: false
#| include: true
