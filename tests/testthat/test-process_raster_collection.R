...
testthat::test_that("process_weighted_raster_collection handles custom parameters", {
  # create temporary directories
  temp_dir <- tempdir()
  value_dir <- file.path(temp_dir, "values")
  pop_dir <- file.path(temp_dir, "population")
  ...
  # create value file with custom extension
  value_file <- file.path(value_dir, "custom_data_2020.tiff")
  raster_data <- terra::rast(
    nrows = 2,
    ncols = 2,
    xmin = 0,
  ...
  # mock batch_extract_weighted_mean with parameter verification
  mock_weighted_function <- function(
    value_raster_file,
    pop_raster_file,
    shapefile,
    id_cols,
    ...
    # verify custom parameters are passed through
    testthat::expect_equal(id_cols, c("country_name", "region_name"))
    testthat::expect_equal(value_layer_to_process, 2)
    testthat::expect_false(weight_na_as_zero)

    data.frame(
      file_name = basename(value_raster_file),
      country_name = shapefile$country_name[1],
      region_name = shapefile$region_name[1],
      year = 2020,
      weighted_mean = 0.25
    )
  }
  ...
  result <- process_weighted_raster_collection(
    ...
    pop_raster_dir = pop_dir,
    shapefile = mock_shapefile,
    id_cols = c("country_name", "region_name"),
    value_pattern = "\\.tiff$",
    pop_pattern = "\\.tif$",
    value_layer_to_process = 2,
    weight_na_as_zero = FALSE
  )

  # check custom columns are included
  testthat::expect_true(all(
    c("country_name", "region_name", "weighted_mean") %in% names(result)
  ))
  testthat::expect_equal(result$country_name, "test_country")
  testthat::expect_equal(result$region_name, "test_region")
  testthat::expect_equal(result$weighted_mean, 0.25)
